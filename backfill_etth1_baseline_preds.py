...
def build_args() -> Namespace:
    return Namespace(
        is_training=0,
        model_id="etth196_validate_baseline_itr3",
        model="iTransformer",
        data="ETTh1",
        root_path="./dataset/",
        data_path="ETTh1.csv",
...
def main() -> None:
    ...
    for setting in settings:
        ...
        print(f"[Backfill] {setting}", flush=True)
        exp = Exp_Long_Term_Forecast(args)
        exp.model.load_state_dict(torch.load(ckpt, map_location=exp.device))
        result_dir = REPO / "results" / setting
        result_dir.mkdir(parents=True, exist_ok=True)
        for flag, pred_name, true_name, metric_name in [
            ("val", "val_pred.npy", "val_true.npy", "val_metrics.npy"),
            ("test", "pred.npy", "true.npy", "metrics.npy"),
        ]:
            pred, true = collect_split_predictions(exp, flag=flag)
            ...
            print(
                f"[Backfill] {flag} | mse={mse:.6f} mae={mae:.6f} shape={pred.shape}",
                flush=True,
            )
