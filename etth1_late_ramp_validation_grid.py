...
def evaluate_schedule(
    *,
    ...
    for projection, directory in enumerate(static_dirs):
        ...
        b_static_mse, b_static_mae = mse_mae(static_err[bucket5])
        b_post_mse, b_post_mae = mse_mae(post_err[bucket5])
        projection_rows.append(
            {
                ...
                "bucket5_static_mae": b_static_mae,
                "bucket5_posthoc_mae": b_post_mae,
            }
        )
    ...
    bucket5_static_mae = float(df["bucket5_static_mae"].mean())
    bucket5_post_mae = float(df["bucket5_posthoc_mae"].mean())
    return {
        "q_low": q_low,
        "q_high": q_high,
        ...
        "bucket5_static_mae": bucket5_static_mae,
        "bucket5_posthoc_mae": bucket5_post_mae,
        "bucket5_mae_gain_abs": bucket5_static_mae - bucket5_post_mae,
        "bucket5_mae_gain_pct": 100.0 * (bucket5_static_mae - bucket5_post_mae) / bucket5_static_mae,
    }
...
def evaluate_grid(
    ...
    pred_file: str,
    true_file: str,
    schedules: list[tuple[float, float, float, float]],
) -> pd.DataFrame:
...
def main() -> None:
    ...
    gamma_min = 0.03
    gamma_maxs = [0.04, 0.05, 0.06, 0.08]
    schedules = [
    ...
    val_path = OUT_DIR / "etth196_staticcausal_late_ramp_val_grid.csv"
    ...
    test_path = OUT_DIR / "etth196_staticcausal_late_ramp_test_selected.csv"
    test_df.to_csv(test_path, index=False)

    top_path = OUT_DIR / "etth196_staticcausal_late_ramp_val_top10.csv"
    ...
    print(
        best[
            [
                ...
                "posthoc_mae",
                "mae_gain_pct",
                "bucket5_mse_gain_pct",
                "bucket5_mae_gain_pct",
                "gamma_mean",
                "gamma_non_min_frac",
            ]
        ].to_string()
