...
def sample_deltaA_strength_change(A_base: np.ndarray, K_edges: int = 6, gain: float = 2.5, flip_prob: float = 0.3):
    """
    Choose K existing cross-variable edges in A_base and change their strength in regime1.
    DeltaA = A_regime1 - A_base (non-zero only on changed edges).
    """
    L, N, _ = A_base.shape
    ...
    for l in range(L):
        for tgt in range(N):
            for src in range(N):
                if src == tgt:
                    continue
                if abs(float(A_base[l, tgt, src])) > 1e-8:
                    existing.append((l, tgt, src))

    np.random.shuffle(existing)
    chosen = existing[:K_edges]

    for (l, tgt, src) in chosen:
        w = float(A_base[l, tgt, src])
        if np.random.rand() < flip_prob:
            w_new = -gain * w
        else:
            w_new = gain * w
        DeltaA[l, tgt, src] = np.float32(w_new - w)

    return DeltaA

def simulate_regime_switch_tanh(
    T: int, A0: np.ndarray, A1: np.ndarray, t_switch: int,
    sigma: float = 0.25, burnin: int = 200, gamma: float = 3.0
):
    L, N, _ = A0.shape
    total_T = T + burnin
    ...
    for t in range(L, total_T):
        ...
        for l in range(L):
            z += A[l] @ X[t - (l + 1)]
        X[t] = np.tanh(gamma * z) + np.random.normal(0, sigma, size=(N,)).astype(np.float32)
...
def main():
    ...
    K_change_edges = 6     # 变化边数量（建议 6 或 8）
    gain = 2.5             # 强度放大倍数
    flip_prob = 0.3        # 翻符号概率（机制反转）
    # ----------------

    A_base = make_sparse_var_params(N=N, L=L, indeg=indeg)
    A_base = stabilize_var_params(A_base, target_rho=0.95, max_iter=8)

    DeltaA = sample_deltaA_strength_change(A_base, K_edges=K_change_edges, gain=gain, flip_prob=flip_prob)
    ...
    # changed-edge ground truth (collapsed):
    adj_change = (np.abs(DeltaA).sum(axis=0) > 1e-8).astype(np.int32)
    ...
    np.save(os.path.join(out_dir, "adj_change.npy"), adj_change)

    meta = dict(
        N=N, L=L, T=T, t_switch=t_switch, switch_ratio=switch_ratio,
        indeg=indeg, sigma=sigma, gamma=gamma, burnin=burnin,
        K_change_edges=K_change_edges, gain=gain, flip_prob=flip_prob
    )
    ...
    print(f"Edges base={int(adj_base.sum())}, regime1={int(adj_regime1.sum())}, changed_edges={int(adj_change.sum())}")
