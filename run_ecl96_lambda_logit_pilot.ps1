...
$logDir = Join-Path $repo 'logs\ecl96_lambda_logit_pilot_20260421'
...
$experiments = @(
  @{ name='01_baseline'; args=@('--model_id','ecl96_clean_baseline_itr1','--des','ECL96CleanBase') },
  @{ name='02_static_anchor'; args=@('--model_id','ecl96_clean_static_anchor_itr1','--des','ECL96CleanAnchor') + $graphCommon },
  @{ name='03_lambda_favor_base'; args=@('--model_id','ecl96_clean_lambda_favor_base_itr1','--des','ECL96LambdaFavorBase') + $graphCommon + @('--graph_lambda_logit_bias','True','--graph_lambda_logit_bias_polarity','favor_base') },
  @{ name='04_lambda_favor_static'; args=@('--model_id','ecl96_clean_lambda_favor_static_itr1','--des','ECL96LambdaFavorStatic') + $graphCommon + @('--graph_lambda_logit_bias','True','--graph_lambda_logit_bias_polarity','favor_static') }
)
